 ...
 */
var isOneEdit = function (str, oneEdit) {
    // Check if a string is oneEdit (Add or Remove)
    if (Math.abs(oneEdit.length - str.length) > 1)
        return false;
    // Check if a string is bigger than the other (Update)
    if (oneEdit.length > str.length)
        return oneEdit.includes(str);
    if (str.length > oneEdit.length)
        return str.includes(oneEdit);
...
var teste = isOneEdit("victor", "victor");
